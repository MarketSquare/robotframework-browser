...
docker-test:
	rm -rf atest/output
	docker run --rm --ipc=host --security-opt seccomp=atest/docker/chrome.json -v $(shell pwd)/atest/:/atest -v $(shell pwd)/node/:/node/ -it rfbrowser sh -c "PATH=$$PATH:~/.local/bin pabot --pabotlib --loglevel debug --exclude Not-Implemented --outputdir /atest/output /atest/test"
