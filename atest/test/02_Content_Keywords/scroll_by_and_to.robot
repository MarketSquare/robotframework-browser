...
Check Visibility
    [Arguments]    ${selector}
    ${s}=    Get Client Size
    Get BoundingBox    ${selector}    ALL
    ...    validate    value.x >= 0 and value.y >= 0 and value.width+value.x <= $s.width and value.height+value.y <= $s.height
...
Scroll To Scroll By
    ...
    Get Scroll Position    ${Selector}    ALL    validate    value.left > 400 and value.top > 400 and value.bottom < $s.height-400 and value.right < $s.width-400
    Scroll To    ${Selector}    top    left
