...
Compare Translation Files
    ...
    Should Match Regexp    ${lines}[0]    ${re_prefix}Found differences between translation and library, see below for details\.
    Should Match Regexp    ${lines}[1]    ${re_prefix}| Keyword name$${SPACE * 19}| Reason${SPACE * 24}|
    Should Match Regexp    ${lines}[2]    ${re_prefix}| ------------------------------ | ----------------------------- |
    Should Match Regexp    ${lines}[3]    ${re_prefix}| cancel_download${SPACE * 20}| Documentation update needed${SPACE * 4}|
    Should Match Regexp    ${lines}[4]    ${re_prefix}| close_page${SPACE * 25}| Keyword is missing translation |
    Should Match Regexp    ${lines}[5]    ${re_prefix}| grant_permissions${SPACE * 18}| Keyword is missing translation |
    Should Match Regexp    ${lines}[6]    ${re_prefix}| focus${SPACE * 30}| Documentation update needed${SPACE * 4}|
    Should Match Regexp    ${lines}[7]    ${re_prefix}| __intro__${SPACE * 26}| Documentation update needed${SPACE * 4}|
