...
export async function internalClick(
    ...
    try {
        ...
        if (error instanceof Error && error.message.startsWith('Target page, context or browser has been closed')) {
            logger.warn('Supress Target page, context or browser has been closed error');
            return false;
        }
        throw error;
    }
