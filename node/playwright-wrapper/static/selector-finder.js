...
function cssesc(a, b = {}) {
    ...
    return (
        e && (/^-[-\d]/.test(g) ? (g = '\\-' + g.slice(1)) : /\d/.test(f) && (g = '\\3' + f + ' ' + g.slice(1))),
...
>Cancel</button>
                ...
                document.getElementById(BROWSER_LIBRARY_DESCRIPTION).textContent =
                    `${Math.round(300 - timediff / 10) / 100} s`;
            }
        }
        ...
        function displaySelection(focusDiv) {
            ...
            const div =
                htmlToElement(`<div id="${BROWSER_LIBRARY_SELECTION_PANEL}" class="${BROWSER_LIBRARY_SELECTOR_CLASS}" style="
    display: flex;
